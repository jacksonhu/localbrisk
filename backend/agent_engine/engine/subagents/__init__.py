...
- 优先使用参数化查询，不拼接不可信输入；
- 表名/字段名/排序字段必须做白名单校验；
- 不执行破坏性 SQL（DROP/TRUNCATE/DELETE 无审批）。"""
...
def _as_compiled_subagent_or_fallback(
    *,
    ...
    """优先创建 CompiledSubAgent，失败则回退为字典定义。"""
    try:
        ...
        logger.warning("CompiledSubAgent 不可用，回退字典定义: %s", e)
...
def create_builtin_subagents(
    *,
    ...
    - `ppt_summary_agent` 优先使用 CompiledSubAgent（可细粒度控制），不可用时自动回退字典；
    - 三者均复用父级模型/工具/沙箱后端，保证在同一运行环境中执行。
    """
    subagents: List[Any] = [
        _as_dict_subagent(
