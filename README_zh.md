...
| **后端服务** | Python FastAPI + OpenAI Agents SDK + 共享 agent context loader + DeepAgents 兼容路径 + Polars + DuckDB | AI Agent 运行时、数据计算、资产管理、模型网关 |
...
- **主运行时**：`AgentRuntimeService` 默认通过 `OpenAIAgentsEngine` 构建与执行 Agent。
- **共享构建上下文**：`agent_context_loader.py` 统一解析 `agent_spec.yaml`、active model、启用的 memories、skills、挂载的 asset bundles，以及可写的 `output/` 工作区。
- **会话持久化**：每个 Agent 的 OpenAI SDK 会话保存在 `output/.openai_sessions.sqlite`，对话历史写入 `output/.chathistory/`。
- **兼容路径**：`DeepAgentsEngine` 仍保留，用于渐进清理、测试迁移和兼容性工作，但已不再是默认运行时。

### 配置热重载机制

- 运行时状态会记录每个已加载 Agent 的 `config_fingerprint`。
- 复用运行时实例前，会重新计算 `agent_spec.yaml`、`models/`、`memories/`、`skills/`、`prompts/` 以及 BusinessUnit `asset_bundles/` 元数据的配置指纹。
- 如果指纹变化且 Agent 当前为 **READY**，则下一次请求会自动重建 runtime。
- 如果 Agent 当前为 **RUNNING**，则会先保留本次执行，新的配置会在下一次请求时生效。

### Handoff、工具与流式协议

- 内置工具会绑定到 agent workspace backend。
- 现有内置 subagent 注册表会通过 `handoff_registry.py` 转换为 **OpenAI Agents handoffs**。
- 流式执行结果会统一归一到 `StreamMessage` 协议，前端继续按 `THOUGHT`、`TASK_LIST`、`ARTIFACT`、`STATUS`、`ERROR`、`DONE` 渲染。

### 相关文档

- `backend/architecture.md` — 后端架构与运行时边界
- `docs/agent_engine_design.md` — Agent 框架设计与构建流程
- `docs/API.md` — 当前后端 API 总览，含 runtime 端点
- `docs/deepagents_sdk_guide.md` — DeepAgents 兼容说明与迁移映射
- `docs/agent_spec_example.yaml` — 最新的 agent spec 示例
...
当前处于 **Phase 5 — 本体建模** 阶段。查看完整路线图（含详细任务拆解与进度跟踪）：
